...
import { Package, Scale, Droplets, Leaf, Wrench, Calendar, Weight } from 'lucide-react';
import { Product, ProductVariant } from '@/types';

interface ExtendedProductSpecsProps {
  productId: string;
  specs: {
    weight: string;
    dimensions: string;
...
export function ExtendedProductSpecs({ specs, productId, variants }: ExtendedProductSpecsProps) {
  // Get the first variant to access additional spec fields
      ...
      <CardContent>
        <Table>
          <TableBody>
            ...
            {/* Net Weight */}
            {(specs.netWeight || mainVariant?.net_weight) && (
              <TableRow>
                <TableCell className="font-medium">Net Weight</TableCell>
                <TableCell>{specs.netWeight || `${mainVariant?.net_weight} ${mainVariant?.weight_unit || ''}`}</TableCell>
              </TableRow>
            )}
            
            {/* Gross Weight */}
            {(specs.grossWeight || mainVariant?.gross_weight) && (
              <TableRow>
                <TableCell className="font-medium">Gross Weight</TableCell>
                <TableCell>{specs.grossWeight || `${mainVariant?.gross_weight} ${mainVariant?.weight_unit || ''}`}</TableCell>
              </TableRow>
            )}
            
            {/* Net Content */}
            {(specs.netContent || mainVariant?.net_content) && (
              <TableRow>
                <TableCell className="font-medium">Net Content</TableCell>
                <TableCell>{specs.netContent || mainVariant?.net_content}</TableCell>
              </TableRow>
            )}
